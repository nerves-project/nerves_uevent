...
defmodule NervesUEvent.InputId do
  ...
  # Any one of these keys or absolute axes signals a joystick.
  @joystick_key_bits [@btn_trigger, @btn_a, @btn_1]
  @joystick_abs_bits [
    @abs_rx,
    @abs_ry,
    @abs_rz,
  ...
  The kvmap has the information needed for classification, but it's
  in bitmaps. This decodes the bitmaps.
  """
  ...
  defp pointer_classes(ev, key, abs_, rel, prop, has_abs_xy) do
    ...
    r =
      initial
      |> from_abs(has_abs_xy, ctx)
      |> from_mt(mt_xy?, ctx)

    mouse? = r.mouse or (rel_xy? and ctx.mouse_button?)
  ...
  defp pointer_ctx(key, abs_, prop) do
  ...
  defp from_abs(r, false, _ctx), do: r

  defp from_abs(r, true, ctx) do
    cond do
      tablet?(ctx) -> %{r | tablet: true}
      touchpad?(ctx) -> %{r | touchpad: true}
      ctx.mouse_button? -> %{r | mouse: true}
      touchscreen?(ctx) -> %{r | touchscreen: true}
      joystick?(ctx) -> %{r | joystick: true}
      true -> r
    end
  end

  defp from_mt(r, false, _ctx), do: r

  defp from_mt(r, true, ctx) do
    cond do
      tablet?(ctx) -> %{r | tablet: true}
      touchpad?(ctx) -> %{r | touchpad: true}
      touchscreen?(ctx) -> %{r | touchscreen: true}
  ...
  defp tablet?(ctx), do: ctx.stylus? or ctx.pen?
  defp touchpad?(ctx), do: ctx.finger_but_no_pen? and not ctx.direct?
  defp touchscreen?(ctx), do: ctx.touch? or ctx.direct?

  defp joystick?(ctx) do
    Enum.any?(@joystick_key_bits, &bit?(ctx.key, &1)) or
      Enum.any?(@joystick_abs_bits, &bit?(ctx.abs_, &1))
  end
